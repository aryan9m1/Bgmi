...
bot = telebot.TeleBot('6419597877:AAG59HyLIq4A3Z40ruO9nXIL0wqlMXeQHtc') 
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"𝙔𝙊𝙐𝙍 𝘼𝙏𝙏𝘼𝘾𝙆 𝙎𝙏𝘼𝙍𝙏𝙀𝘿 𝙎𝙁𝙐𝙇𝙇𝙔 𝘽𝙔 @BGS_AYUSH  ✅ " 
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 1: 
                response = "𝙔𝙊𝙐 𝘼𝙍𝙀 ON 𝘾𝙊𝙊𝙇𝘿𝙊𝙒𝙉 𝙒𝘼𝙄𝙏 1 𝙎𝙀𝘾𝙊𝙉𝘿𝙎 ⏳" 
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 341:
                response = "❌ 𝙀𝙍𝙍𝙊𝙍 𝙐𝙎𝙀 𝙇𝙀𝙎𝙎 𝙏𝙃𝙀𝙉 341 𝙎𝙀𝘾𝙊𝙉𝘿𝙎 ❌."
            else:
                ...
                response = f"𝘽𝙂𝙈𝙄 𝘿𝘿𝙊𝙎 𝘼𝙏𝙏𝘼𝘾𝙆 𝙁𝙄𝙉𝙄𝙎𝙃𝙀𝘿 𝘽𝙔 @BGS_AYUSH 🔥" 
        else:
...
def welcome_start(message):
    ...
    response = f"🙏🏻 𝙒𝙀𝙇𝘾𝙊𝙈𝙀 𝙏𝙊 VIP AYUSH TESM 𝙃𝙐𝘽 𝘼𝙇𝙇 𝘾𝙊𝙈𝙈𝘼𝙉𝘿 𝘼𝙑𝘼𝙄𝙇𝘼𝘽𝙇𝙀 𝙊𝙉 𝙈𝙀𝙉𝙐. 𝙅𝙊𝙄𝙉 𝙊𝙐𝙍 𝘾𝙃𝘼𝙉𝙉𝙀𝙇 @BGS_AYUSH" 
    bot.reply_to(message, response)
